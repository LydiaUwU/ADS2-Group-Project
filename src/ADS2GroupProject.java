    ...
    public static void main(String[] args) {
        ...
        while (outerLoop) {
            ...
            if (input.equals("quit")) outerLoop = false;
                ...
                while (innerLoop)
                {
                    System.out.println("Enter ID of first stop (or type \"back\" to go back):");
                    input = inputScanner.next();
                    if (input.equals("back")) innerLoop = false;
                    else {
                        sourceStop = Integer.parseInt(input);

                        // If node/stop/whatever exists
                        if (stopsGraph.getNodeIndexFromLabel(sourceStop) >= 0) {
                            System.out.println("Enter ID of second stop (or type \"back\" to go back):");
                            input = inputScanner.next();
